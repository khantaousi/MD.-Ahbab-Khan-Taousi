...
const AIChatBot: React.FC<AIChatBotProps> = ({ accentColor, data, isLightMode = false, showBackToTop = false }) => {
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-8 ${showBackToTop ? 'right-24' : 'right-8'} w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-110 z-[100] ${isOpen ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`}
